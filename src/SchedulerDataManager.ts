...
import { TimePeriods, CellUnits, ViewTypes } from './enum';
...
export class SchedulerDataManger {
  ...
  private calculateDataUnitLength() {
    if (
      ...
      this.dimensions.dataUnitLength = Number(
        (this.dimensions.dataLength / this.xAxis.length).toFixed(2),
      );
      this.dimensions.minimumDataUnitLength =
        this.timePeriod === TimePeriods.Week
          ? Number((this.dimensions.dataLength / (7 * 24)).toFixed(2))
          : this.timePeriod === TimePeriods.Month
          ? Number((this.dimensions.dataUnitLength / (this.xAxis.length * 24)).toFixed(2))
          : Number((this.dimensions.dataLength / (24 * 60)).toFixed(2));
    } else {
      this.dimensions.dataUnitLength = this.config.defaultDataUnitLength;
      this.dimensions.minimumDataUnitLength = Number(
        (this.dimensions.dataUnitLength / (this.xAxis.length * 24)).toFixed(2),
      );
    }
  }
    ...
    if (this.timePeriod === TimePeriods.Day) {
        ...
        for (let i = 0; i < minuteSteps; i++) {
          ...
          if (hour >= this.startTimeOfDay && hour <= this.endTimeOfDay) {
            ...
            this.xAxis.push({
              endTime,
              length:
                hour === this.endTimeOfDay
                  ? this.dimensions.dataLength - 11 * this.dimensions.dataUnitLength
                  : this.dimensions.dataUnitLength,
              startTime,
              workingTime,
            });
          }
      ...
      while (start.isSameOrBefore(end)) {
        ...
        this.xAxis.push({
          endTime,
          length:
            start.daysInMonth() === end.daysInMonth()
              ? this.dimensions.dataLength -
                (end.daysInMonth() - 1) * this.dimensions.dataUnitLength
              : this.dimensions.dataUnitLength,
          startTime,
          workingTime,
        });
  ...
  private getCellMaxEvents() {
    return this.timePeriod === TimePeriods.Week
  ...
  private recalXAxisLength() {
    if (
      ...
      this.xAxis.forEach((x, indx) => {
        x.length =
          indx === this.xAxis.length - 1
            ? this.dimensions.dataUnitLength -
              (this.xAxis.length - 1 * this.dimensions.dataUnitLength)
            : this.dimensions.dataUnitLength;
      });
    }
  }
