...
export class SchedulerDataManger {
  ...
  /**
   * Generate scheduler header.
   */
  private generateHeaders() {
    this.headers = [];
    const start = this.localeMoment(this.startDate.format(DATE_FORMAT));
    const end = this.localeMoment(this.endDate.format(DATE_FORMAT));
    let header = start;

    if (this.showAgenda) {
      this.headers.push({ time: this.startDate.format(DATETIME_FORMAT), workingTime: false });
    } else if (this.cellUnit === CellUnits.Hour) {
        ...
        for (let i = 0; i < minuteSteps; i++) {
          ...
          if (hour >= this.startTimeOfDay && hour <= this.endTimeOfDay) {
            ...
            this.headers.push({
              ...
              workingTime: isWorkingTime(time, this.localeMoment, this.cellUnit),
            });
          }
